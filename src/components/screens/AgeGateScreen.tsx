 ...
 *
 * Écrite dans la PR #97 (2026-08-06) et restée en attente cinq semaines pendant
 * que la direction artistique changeait sous elle. Portée telle quelle le
 * 2026-09-13 : seuls les jetons ont bougé - `bg-pop-*` et `shadow-tile-lg`
 * n'existent plus depuis « Tirage de nuit », `min-h-screen` a cédé la place à
 * `min-h-dvh` pour la barre d'adresse mobile. Le raisonnement, lui, n'a pas
 * bougé d'une ligne.
 *
...
export function AgeGateScreen() {
  ...
  if (reponse === 'mineur') {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6 pt-safe pb-safe bg-bg">
        <div className="w-full max-w-md rounded-card border-2 border-tile-ink bg-aplat-3 p-8 text-center text-tile-ink shadow-card-elevated">
        ...
        <p className="mt-6 max-w-md text-center font-sans text-xs text-ink-secondary">
  ...
  return (
    <div className="min-h-dvh flex flex-col px-6 pt-safe pb-safe bg-bg">
      <main className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full">
        <motion.div
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ type: 'spring', damping: 22, stiffness: 180 }}
          className="w-full rounded-card border-2 border-tile-ink bg-aplat-1 p-8 text-center text-tile-ink shadow-card-elevated"
        >
          <Icon name="balance" className="w-12 h-12 mx-auto mb-5 text-tile-ink" aria-hidden="true" />
