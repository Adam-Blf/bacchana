    ...
    >
      ...
      <header className="flex-shrink-0 mb-4 pt-16 relative z-10 text-center">
        ...
        <div className="mt-2 inline-flex items-center gap-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-pill bg-pop-yellow text-tile-ink border-2 border-ink font-mono text-xs font-bold tabular-nums">
        ...
        <AnimatePresence mode="wait">
          {session.phase === 'question' && session.currentQuestion && (
            ...
            >
              <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
                <span className="px-3 py-1 rounded-pill bg-pop-blue text-tile-ink border-2 border-ink font-mono text-xs font-bold uppercase">
                  {session.currentQuestion.category}
                </span>
                <span className="px-3 py-1 rounded-pill bg-pop-pink text-tile-ink border-2 border-ink font-mono text-xs font-bold tabular-nums">
              ...
              {answerShown ? (
                <motion.p
                  ...
                  className="mt-5 font-sans text-base bg-pop-lime text-tile-ink border-2 border-ink rounded-control px-4 py-3"
                  aria-live="polite"
                >
          ...
          {session.phase === 'choice' && (
            <motion.div
              ...
              animate={{ y: 0, opacity: 1, scale: 1 }}
              exit={{ y: -30, opacity: 0, scale: 0.96 }}
              className="w-full max-w-md rounded-card p-6 sm:p-8 bg-pop-yellow text-tile-ink border-2 border-ink shadow-card-elevated text-center"
            >
