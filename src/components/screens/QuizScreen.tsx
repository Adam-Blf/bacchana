    ...
    >
      ...
      <header className="flex-shrink-0 mb-4 pt-16 relative z-10 text-center">
        ...
        <div className="mt-2 inline-flex items-center gap-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-pill bg-pop-yellow border-2 border-on-pop text-on-pop font-mono text-xs font-bold tabular-nums">
        ...
        <AnimatePresence mode="wait">
          {session.phase === 'question' && session.currentQuestion && (
            <motion.div
              ...
              className="w-full max-w-md rounded-card p-6 sm:p-8 bg-card-face text-card-ink border-2 border-ink shadow-card-elevated text-center"
            >
              <div className="flex items-center justify-center gap-2 mb-4 flex-wrap">
                <span className="px-3 py-1 rounded-pill bg-pop-blue border-2 border-on-pop text-on-pop font-mono text-xs font-bold uppercase">
                  {session.currentQuestion.category}
                </span>
                <span className="px-3 py-1 rounded-pill bg-pop-pink border-2 border-on-pop text-on-pop font-mono text-xs font-bold tabular-nums">
              ...
              {answerShown ? (
                <motion.p
                  ...
                  className="mt-5 font-sans text-base bg-pop-lime border-2 border-on-pop text-on-pop rounded-control px-4 py-3"
                  aria-live="polite"
                >
          ...
          {session.phase === 'choice' && (
            <motion.div
              ...
              animate={{ y: 0, opacity: 1, scale: 1 }}
              exit={{ y: -30, opacity: 0, scale: 0.96 }}
              className="w-full max-w-md rounded-card p-6 sm:p-8 bg-pop-yellow border-2 border-on-pop shadow-card-elevated text-center"
            >
              <Brain className="w-8 h-8 mx-auto mb-3 text-on-pop" aria-hidden="true" />
              <p className="font-display text-2xl uppercase tracking-tight text-on-pop">
                Bien joué !
              </p>
              <p className="font-sans text-on-pop/80 mt-2">
