...
import { Share2, Home, RotateCcw } from '@/components/ui/icons'
...
export function SessionRecap({
  ...
  const handleShare = async () => {
    ...
    const text = `La Tournée - l'addition\n\n${lines.join('\n')}\n\nTotal : ${totalGorgees} pénalités${
      penaltyCounts ? '' : `, ${totalShots} majeures`
    } distribuées.\nlatournee.beloucif.com`
    try {
      if (navigator.share) {
        await navigator.share({ title: "La Tournée - L'addition", text })
      } else {
      ...
      >
        <div className="px-5 pt-7 pb-8 text-[13px] leading-relaxed">
          ...
          <div className="text-center">
            <div className="font-bold text-lg tracking-wide uppercase">La Tournée</div>
            <div className="text-[11px] text-[#6e6759]">Au coin du comptoir - Chevilly-Larue</div>
            <div className="text-[11px] text-[#6e6759]">latournee.beloucif.com</div>
          </div>
